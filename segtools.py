...
def get_params_median_mean(
    labels: np.ndarray, parameter: str = "feret_diameter_max"
) -> Optional[ObjParams]:
    ...
    try:
        ...
        objparams = ObjParams(parameter, np.median(values_array), values_array.mean())

    except Exception as e:
        print("Parameter:", parameter, " does not exist.")
        objparams = ObjParams(parameter, None, None)

    return objparams


def apply_watershed(
    binary: np.ndarray, estimate_min_distance: bool = False, min_distance: int = 10
) -> np.ndarray:
    ...
    if estimate_min_distance:
        feretmax = get_params_median_mean(binary, parameter="feret_diameter_max")
...
def apply_watershed_adv(
    ...
    if estimate_min_distance:
        feretmax = get_params_median_mean(segmented, parameter="feret_diameter_max")
...
def autoThresholding(
    image2d: np.ndarray, method="triangle", radius=10, value=50
) -> np.ndarray:
...
def subtract_background(image, elem="disk", radius=50, light_bg=False) -> np.ndarray:
...
def erode_labels(
    segmentation: np.ndarray, erosion_iterations: int, relabel=True
) -> np.ndarray:
    ...
    for i in range(len(regions)):
        ...
        list_of_eroded_masks.append(
            erode_mask(segmentation, label_id, erosion_iterations, relabel=relabel)
        )
...
def area_filter(
    im: np.ndarray, area_min: int = 10, area_max: int = 100000
...
def filter_labels(
    labels: np.ndarray, min_size: int, max_size: int
) -> Tuple[np.ndarray, int]:
